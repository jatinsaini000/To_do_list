import { jatin_backend } from "../../declarations/jatin_backend";
...
appointForm.addEventListener("submit", async (e) => {
  ...
  try {
    const getId = await jatin_backend.addTodo(obj.textarea);
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    const getAll = await jatin_backend.getAllTodo();
...
async function deleteTodo(userId) {
  try {
    ...
    await jatin_backend.deleteTodo(userId);
